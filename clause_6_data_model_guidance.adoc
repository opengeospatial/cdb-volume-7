...
The goal of this guidance is to model a typical high voltage electrical pylon resembling the one in the following figure. This guidance is based originally on CDB version 3.1 prior to submission to the OGC, but is applicable to version 3.0 as well footnote:[See the OGC CDB Best Practice. That document is version 3.x of the CDB specification as originally submitted to the OGC and is the basis for the formal, OGC Member approved CDB Standard and Best Practices starting at OGC Version 1.0].
...
ifndef::backend-pdf[]
latexmath:[E_{\text{A}}] = latexmath:[E_{\text{LA}} * Ain_{\text{LA}} + E_{\text{A}} * Aout_{\text{LA}} ]
endif::[]
ifdef::backend-pdf[]
